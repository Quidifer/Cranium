...
export default class Engine {
  ...
  /*
    ...
    */
  public async calculateMoveSet_Transfer(
    ship: FrontEndManifest,
    onloads: Array<Onload>,
    offloads: Array<Offload>
  ): Promise<Solution> {
    ...
    const CostMap: Array<CostMapType> = ship
      .flatMap((container) => {
        return {
          container: container,
          cost: this.Manhatten_Distance(
            { row: container.row, col: container.col },
            this.SHIP_VIRTUAL
          ),
        };
      })
    ...
    for (let i = 0; i < offloads.length; i++) {
      ...
      const offloadCost = this.Manhatten_Distance(
        { row: offload.row, col: offload.col },
        this.SHIP_VIRTUAL
      );
      for (let j = 0; j < CostMap.length; j++) {
        let costItem = CostMap[j];
        if (
          costItem.container.name === offload.name &&
          (costItem.container.row !== offload.row ||
            costItem.container.col !== offload.col) &&
          costItem.cost < offloadCost &&
          offloads
            .filter((o) => Object.values(o) !== Object.values(offload))
            .every(
              (o) =>
                costItem.container.row !== o.row ||
                costItem.container.col !== o.col
            )
        ) {
    ...
    const initialState = new EngineNode(
      convertedShip,
      initial_buffer,
      onloads,
      offloads
    ...
    do {
      ...
      this.engineLog(
        `Expanding EngineNode (Ons: ${leaf.onloads.length}, Offs: ${leaf.offloads.length}): ${leaf.id}`
      );
      ...
      if (leaf.offloads.length === 0) {
          ...
          for (let col = 0; col < this.SHIP_DIMENSIONS.COLUMN_MAX; col++) {
            for (let row = 0; row < this.SHIP_DIMENSIONS.ROW_MAX; row++) {
              ...
              if (newNode.state[row][col] === null) {
                ...
                let new_cost = this.Manhatten_Distance(
                  { row: row + 1, col: col + 1 },
                  this.SHIP_VIRTUAL
                );
                let new_minutes = new_cost + newNode.minutes;
                let new_state = cloneDeep(newNode.state);
                new_state[row][col] = this.moveContainer(
                  Object.assign(onload, {
                    row,
                    col,
                ...
                new_leaf.cost =
                  ...
                  new_cost +
                  new_depth +
                  this.heuristic(new_leaf);
                new_leaf.depth = new_depth;
                new_leaf.previousNode = newNode;
                newNode = new_leaf;
                this.engineLog(
                  `Onloading a container named '${onload.name}' to (${
                    row + 1
                  },${col + 1})`
                );
              }
        ...
        while (!newNode.bufferIsEmpty()) {
          ...
          for (let col = 0; col < this.SHIP_DIMENSIONS.COLUMN_MAX; col++) {
            for (let row = 0; row < this.SHIP_DIMENSIONS.ROW_MAX; row++) {
              if (newNode.state[row][col] === undefined) continue;
              if (newNode.state[row][col] === null) {
                let container = newNode.buffer
                  .flatMap((r) => r)
                  .filter((container) => container !== null)[0];
                if (container === null || container === undefined) break;

                let new_depth = 0;
                let new_cost =
                  this.Manhatten_Distance(
                    { row: row + 1, col: col + 1 },
                    this.SHIP_VIRTUAL
                  ) +
                  this.Manhatten_Distance(
                ...
                new_state[row][col] = this.moveContainer(
                  Object.assign({ location: ContainerArea.SHIP }, container),
                  row,
                  col
                );
                new_buffer[container.row - 1][container.col - 1] = null;
                let move = {
                  row_start: container.row,
                  col_start: container.col,
                  row_end: row + 1,
                  col_end: col + 1,
                ...
                newNode = new_leaf;
                this.engineLog(
                  `Moving a container named '${
                    container.name
                  }' from Buffer (Row:${container.row}, Col:${
                    container.col
                  }) to Ship (Row:${row + 1}, Col:${col + 1}).`
                );
              }
            }
          }
        ...
        let final_state = new EngineNode(
          newNode.state,
          [],
          [],
          [],
      ...
      if (leaf.state.every((value) => value === null || value === undefined))
        continue;
      ...
      for (let column = 0; column < this.SHIP_DIMENSIONS.COLUMN_MAX; column++) {
        ...
        for (
          let row_check = 0;
          row_check < this.SHIP_DIMENSIONS.ROW_MAX;
          row_check++
        ) {
        ...
        allOpenCols.forEach((value) => {
          if (
            value < column &&
            value > leftCol &&
            !offloadCols.some((col) => col === value)
          )
            leftCol = value;
          if (
            value > column &&
        ...
        for (
          ...
          containers[row_check] = leaf.state[row_check][column];

        // If OffloadCrate is on top
        const isOffloadCrate = leaf.offloads.some(
          (value) => value.row === topCrate.row && value.col === topCrate.col
        );
        const containsOffloadCrate = containers.some((container) =>
          leaf.offloads.some(
            (offload) =>
              container !== undefined &&
              container !== null &&
              container.row === offload.row &&
              container.col === offload.col
          )
        );
        ...
        if (isOffloadCrate) {
          ...
          let new_offloads = leaf.offloads.filter(
            (value) =>
              !(value.row === topCrate.row && value.col === topCrate.col)
          );

          let new_depth = leaf.depth + 1;
          let new_cost =
            this.COST.TRUCK_MOVE +
            this.Manhatten_Distance(
              { row: topCrate_row + 1, col: column + 1 },
              this.SHIP_VIRTUAL
            );
          let new_minutes = new_cost + leaf.minutes;
          let move = {
            row_start: topCrate_row + 1,
            col_start: column + 1,
            row_end: -1,
            col_end: -1,
          ...
          let new_leaf = new EngineNode(
            new_state,
            cloneDeep(leaf.buffer),
            cloneDeep(leaf.onloads),
            new_offloads,
            move,
            new_minutes
          );
          new_leaf.depth = new_depth;
          new_leaf.cost =
            new_cost + leaf.cost + new_depth + this.heuristic(new_leaf);
          ...
          this.engineLog(`Generating moves for Column ${column + 1}`);
          {
            ...
            for (
              let col_check = 0;
              col_check < this.BUFFER_DIMENSIONS.COLUMN_MAX;
              col_check++
            ) {
              new_col = col_check;
              for (
                let row_check = 0;
                row_check < this.BUFFER_DIMENSIONS.ROW_MAX;
                row_check++
              ) {
                new_row = row_check;
            ...
            let new_cost =
              ...
              this.Manhatten_Distance(
                { row: new_row + 1, col: new_col + 1 },
                this.BUFFER_VIRTUAL
              ) +
              this.COST.AREA_MOVE;
            let new_minutes = new_cost + leaf.minutes;
            new_buffer[new_row][new_col] = this.moveContainer(
              Object.assign({ location: ContainerArea.BUFFER }, topCrate),
              new_row,
              new_col
            );
            let bmove = {
          ...
          openCols.forEach((new_column) => {
            ...
            let new_cost = this.Manhatten_Distance(
              { row: topCrate_row + 1, col: column + 1 },
              { row: new_row + 1, col: new_column + 1 }
            );
            let new_minutes = new_cost + leaf.minutes;
            new_state[new_row][new_column] = this.moveContainer(
              Object.assign({}, topCrate),
              new_row,
              new_column
            );
            let move = {
      ...
      goalStates.forEach(
        (goalState) =>
          (lowestCost =
            goalState.cost < lowestCost ? goalState.cost : lowestCost)
      );
      if (queuedStates.toArray().every((state) => state.cost > lowestCost))
        break;
    ...
    return {
      ...
      moves:
        goalStates.length > 0
          ? this.convertGoalStateToMoves(goalStates[0])
          : undefined,
      final_manifest:
        goalStates.length > 0
          ? this.convertToManifest(goalStates[0].state)
          : undefined,
    };
  }
  ...
  /*
    ...
    */
  public async calculateMoveSet_Balance(
    ship: FrontEndManifest
  ): Promise<Solution> {
    const convertedShip: Ship = this.convertFromFrontEndShip(ship);
    const initial_buffer: Buffer = this.fillBuffer();
    ...
    do {
      ...
      exploredStates.set(md5(leaf.state), "");
      this.engineLog(
        `Expanding EngineNode (WeightDiff: ${this.balanceScore(leaf)}): ${
          leaf.id
        }`
      );
      ...
      for (let column = 0; column < this.SHIP_DIMENSIONS.COLUMN_MAX; column++) {
        ...
        openCols.forEach((new_column) => {
          ...
          console.log(
            `Trying to move a container from ${{
              row: topCrate_row + 1,
              col: column + 1,
            }} to ${{
              row: new_row + 1,
  ...
  public Manhatten_Distance(start: Location, end: Location): number {
    return (
      ...
      this.COST.LOCAL_MOVE
    );
  }

  public Manhatten_Distance_Move(move: CraneMove): number {
    return (
      (Math.abs(move.row_end - move.row_start) +
        Math.abs(move.col_end - move.col_start)) *
      this.COST.LOCAL_MOVE
    );
  }
    ...
    node.state.forEach((row) => {
      row.forEach((cell) => {
        if (cell !== undefined && cell !== null) {
          if (cell.col - 1 < midline) leftSide += cell.weight;
          else {
  ...
  private heuristic(node: EngineNode) {
    ...
    node.buffer.forEach((row) =>
      row.forEach((container) => {
        if (container !== null) buffer.push(container);
      })
    );
    const bufferOnloads = buffer
      .map(
        (container) =>
          this.Manhatten_Distance(
            { row: container.row, col: container.col },
    ...
    const offloadCosts = node.offloads
      .map(
        (offload) =>
          this.Manhatten_Distance(
            { row: offload.row, col: offload.col },
  ...
  private cratesInTheWay(node: EngineNode): number {
    ...
    node.offloads.forEach((offload) => {
      for (
        let rowCount = offload.row;
        rowCount < this.SHIP_DIMENSIONS.ROW_MAX;
        rowCount++
      ) {
  ...
  private convertFromFrontEndShip(ship: FrontEndManifest): Ship {
    let convertedShip: Ship = new Array(this.SHIP_DIMENSIONS.ROW_MAX)
      .fill(undefined)
      .map(() => new Array(this.SHIP_DIMENSIONS.COLUMN_MAX).fill(undefined));
    ...
    ship.forEach((value) => {
      switch (value.name) {
        ...
        default: {
          convertedShip[value.row - 1][value.col - 1] = Object.assign(value, {
            location: ContainerArea.SHIP,
          });
          break;
        }
  ...
  private fillBuffer(): Buffer {
    ...
    for (let row = 0; row < this.BUFFER_DIMENSIONS.ROW_MAX; row++) {
      const newArray = new Array<ShipContainer | null>(
        this.BUFFER_DIMENSIONS.COLUMN_MAX
      ).fill(null);
      buffer[row] = newArray;
    }
    return buffer;
  ...
  private checkForBalanceSolution(ship: Ship): boolean {
    ...
    return (
      Math.min(pile1, pile2) / Math.max(pile1, pile2) > this.BALANCE_SCORE_GOAL
    );
  }
    ...
    while (BufferTargets.length > 0) {
      ...
      for (let move of BufferTargets) {
        ...
        if (point1Available && point2Available) {
          ...
          BufferTargets = BufferTargets.filter(
            (t) => Object.values(t) !== Object.values(move)
          );
        }
      }
      ...
      for (let move of possibleMoves) {
        ...
        let new_cost =
          this.Manhatten_Distance(
            { row: move.a.row + 1, col: move.a.col + 1 },
            this.SHIP_VIRTUAL
          ) +
          this.Manhatten_Distance(
            { row: move.b.row + 1, col: move.b.col + 1 },
            this.BUFFER_VIRTUAL
          ) +
          this.COST.AREA_MOVE;
        let new_minutes = new_cost + newNode.minutes;
        ...
        newBuffer[move.b.row][move.b.col] = this.moveContainer(
          Object.assign({ location: ContainerArea.BUFFER }, move.container),
          move.b.row,
          move.b.col
        );
        let bmove = {
          row_start: move.a.row + 1,
          col_start: move.a.col + 1,
          row_end: move.b.row + 1,
          col_end: move.b.col + 1,
          move_type: CraneMoveType.SHIP_TO_BUFFER,
        ...
        let new_leaf = new EngineNode(
          new_state,
          cloneDeep(newBuffer),
          cloneDeep(newNode.onloads),
          cloneDeep(newNode.offloads),
          bmove,
          new_minutes
        );
        new_leaf.depth = new_depth;
        new_leaf.cost =
          new_cost + newNode.cost + new_depth + this.heuristic(new_leaf);
        new_leaf.previousNode = newNode;
        newNode = new_leaf;
      }
    }
